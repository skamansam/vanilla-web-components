...
import dinnerMenu from "../src/assets/dinnerMenu.json" with { type: "json" };

import lunchMenu from "../src/assets/lunchMenu.json" with { type: "json" };
...
for (const [menu, label, option] of testOptions) {
	test.describe(label, () => {
		...
		for (const [category, categoryInfo] of Object.entries(menu)) {
			...
			test.beforeEach(async ({ page }) => {
				categoryElement = page.locator("category-card").filter({
			...
			test.describe(`Category ${category}`, () => {
				test(`has heading`, async ({ page: _page }) => {
					await expect(
				...
				test(`has description`, async ({ page: _page }) => {
					await expect(
						categoryElement.getByText(categoryInfo.description, {
							exact: true,
						}),
					).toBeVisible();
				});
				...
				for (const item in categoryInfo.items) {
					test.describe(`Item ${item}`, () => {
						test(`has title ${item}`, async ({ page: _page }) => {
							await expect(
								categoryElement.getByRole("heading", {
									name: item,
									level: 4,
